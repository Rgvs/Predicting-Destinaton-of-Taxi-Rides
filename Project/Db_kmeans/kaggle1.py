...
class Dbscan:
    ...
    def db(self):
        ...
        f1 = open('../Kmeans/K_start.txt' ,'r')
        st=f1.read().split('\n')
        for coordinates in st:
            coordinates=coordinates.split()
            #print coordinates[2]
            try:
                start.append ([ float(coordinates[1]),float(coordinates[2][:-1])] )
            except:
                #print coordinates
                pass
        print "TRIP_ID,LATITUDE,LONGITUDE"
        f1 = open('../Kmeans/K_end.txt' ,'r')
        e=f1.read().split('\n')
        for coordinates in e:
            coordinates=coordinates.split()
            try:
                end.append ([ float(coordinates[1]),float(coordinates[2][:-1])] )
            except:
                #print coordinates
                pass
        f1 = open('K_n_transition.txt' ,'r')
        t=f1.read().split('\n')
        ...
        #f1 = open('start_fixed.txt' ,'r')
        #f2 = open('end_fixed.txt' ,'r')
        for line in izip(open('start_n_transition_end_K.txt')):   ####need this fileeeee
        ...
        for tup in self.mongo_test.find( ):
            ...
            trips = trips + 1
            if len(tup["POLY"])==0:
                print str(tup["TRIP_ID"])+','+str(41.146504)+','+str(-8.611317)
                continue
